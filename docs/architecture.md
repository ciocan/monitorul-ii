...
- **`interpellations.py`** (v0.2.0) — `find_interpellation_block` scans for 9 chair-declarative transition phrases; earliest match wins. v0.1 shipped with 5 patterns and a corpus hit-rate of **1 / 5551** docs (most chair openings used phrasings outside the v0.1 set). v0.2.0 widens to: `trecem la primirea răspunsurilor la interpelări`, `Începem sesiunea de întrebări/interpelări` (2009-2017), `Declar deschisă (sesiunea/ședința) (consacrată/de) ... interpelări/întrebări` (modern, 136 hits), `Urmează (prezentarea/sesiunea) ... interpelări` (2009-era), `Deschidem ședința consacrată răspunsurilor`, `Răspunsuri la interpelări.` (line-anchored, rejects pipe-bearing SUMAR table cells via negative lookbehind), `## Întrebări orale...` heading. Combined hit-rate: **254 / 5551** (4.6%, 254× lift). Block ends at EOF (no observed case of agenda content following). Per-interpellation parser uses the same `## **<inner>:**` header pattern as activities; default `genre=interpelare` (more procedural / formal); flips to `întrebare` only when the questioner block strongly hints at oral-question form. `addressed_to` extracted from `adresat[ă] doamnei/domnului ROLE` or `Ministerului X` patterns; v0.1 leaves `addressed_to_normalized` null (deferred to ministry registry per schema § 8 line 231). `interpellation_number` reuses qr's `Nr. N(.NNN)?[A-Z]?` regex (last match wins). `response_deferred=True` when `(în scris)` notation present. **`question_text` (v0.2.0)**: `_extract_question_text(turn_body)` recovers the policy-substance body of the questioner's turn via a 3-step pipeline: (1) cap at the earliest section-break marker (`Voi citi (și) declarația politică ...`, `Trec la a doua/cealaltă (întrebare/interpelare)` — the same speaker continues into a different topic in the same turn); (2) cap at the earliest closing marker (`Solicit răspuns ...`, `Aștept(ăm) răspunsul`, `Doresc un răspuns`, line-anchored `Cu stimă,` / `Cu respect,` signature openers); (3) drop leading lines that are pleasantries (`Mulțumesc, ...`, `Vă mulțumesc.`, `Bună (dimineața/seara/ziua) ...`, `Doamnă/Domnule președinte, ...`, `Stimați colegi/Stimate colege, ...`, `Doamnelor și domnilor, ...`, `Dragi români, ...`) or topic-naming preamble (`Voi (da curs/citi/prezenta) ... (întrebării/interpelării) ...`, `Interpelarea (este adresată/se adresează) ...`, `Întrebarea (este adresată/se adresează) ...`, `Obiectul interpelării: ...`, `Adresez (această) (întrebare/interpelare) ...`). Returns null when residue < 40 chars (boilerplate-only turn — no real content). Production sweep on 51 corpus docs with detected interpellation blocks: 786/867 interpellations (90.7%) recover meaningful question_text. Per-interpellation confidence ramps 0.7 → 0.85 when question_text is filled. `response` remains null (responder-block parsing deferred).
...
Backfill-registry-dependent fields (always null in v0.1): `Speaker.person_id` (person registry), `QuestionAddressee.ministry_normalized` and `Interpellation.addressed_to_normalized` (ministry registry), `Vote.proposed_by` (bill-sponsor registry), `Vote.nominal_breakdown` (parlament.ro per-MP voting feed), `bill.subject` / `law.subject` / `parliamentary_resolution.subject` (best-effort context labels). Schema-modeled but stubbed: 6 long-tail reference variants, `topics.secondary` (LLM pass), per-topic `extraction` provenance block. Interpellation `question_text` parsing **shipped at v0.2.0** (see `interpellations.py` description above; 90.7% recovery on detected blocks); `response` parsing still deferred (requires responder-pattern detection — which `## **NAME** _– role_:**` headers belong to a minister/secretar de stat replying to the prior questioner, not to the chair). Cross-document `defers_to` / `resolves` linker for tying cross-session deferrals to their resolving final-vote document.
