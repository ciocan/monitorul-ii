...
- **`interpellations.py`** (v0.2.2) — full per-doc interpellation pipeline: block detection, header parsing with chair / declaration / footer filtering, `question_text` body recovery, `response` pairing with replying minister turns, and quote-aware `topic` detection. Production sweep on full 5551-doc corpus: 275 docs detect a block (4.96% — vs. v0.1's 1/5551), 7180 interpellations emitted, 6465 (90.0%) recover question_text, 25 paired with a minister response, **0 declaration leakage, 0 footer pollution, 0 salutation-only topics**. Coverage holds at p50=0.998 / mean=0.932.
  ...
  - **Defaults**: `addressed_to` from "adresat[ă] doamnei/domnului ROLE" or "Ministerului X" patterns; `addressed_to_normalized` null (ministry registry deferred); `interpellation_number` via qr's `Nr. N(.NNN)?[A-Z]?` regex (last match wins); `response_deferred=True` when `(în scris)` notation present; default `genre=interpelare`, flips to `întrebare` only when the questioner block strongly hints at oral-question form.
...
| `references` | 0.1.0 (stub) | **0.5.0** | 13 strict variants + `unknown` catch-all (emitter active). v0.2.0 shipped 6 (bill, law, oug, og, chamber_resolution, parliamentary_resolution); v0.3.0 graduated the long-tail set (`motion`, `court_decision`, `constitution`, `regulation`, `eu_doc`, `treaty`); v0.4.0 enabled the `unknown` emitter (hint enum `law-ish` / `court-ish` / `eu-doc-ish` / `other`); v0.5.0 finishes Tier 1 graduations — adds the **`code` variant** (Romanian named codes), **broadens `treaty`** to absorb Acordul/Protocolul/wider Carta forms, and populates **best-effort `subject`** on bill / law / parliamentary_resolution. Schema 1.10.0 adds 1 new $def (`CodeReference`) + `BillReference.subject` field. Production sweep on the 5551-doc corpus: 20,481 code refs graduated; treaty 4,378 → 10,051; unknowns 125,822 → 110,966 (-14,856; `unknown.hint=other` went 3,228 → 0; `unknown.hint=law-ish` went 122,554 → 110,926, dominated by ~96K bare `art. N` cross-references that resist regex graduation). Subject fill rate: bill 50.5%, law 76.8%, parliamentary_resolution 88.9%. Coverage held p50=0.998 / mean=0.932; 12 pre-existing errors unchanged |
