...
5. **fuzzy** — Levenshtein ≤ 2 on the joined-sorted token form (so order doesn't sabotage the distance metric). The only fuzzy tier in any matcher in this codebase; justified for human names per the design doc, capped tightly to keep precision high. Absorbs OCR slips like a missing-letter typo (`Iorache` → `Iordache`, distance 1) without admitting wholesale mistakes.
