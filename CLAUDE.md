...
Python 3.12 + `uv`. CLI `monitorul-ii` with four subcommands: `fetch` (scrape PDFs by date), `convert` (PDF → markdown with extraction-friendly YAML frontmatter), `classify` (type-detect MDs into the v1.5.0 extraction-schema buckets), and `extract` (MD → strict-validated JSON sidecar). Hatchling-backed package at `src/monitorul_ii/`.
