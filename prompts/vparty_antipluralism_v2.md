# V-Party anti-pluralism classifier — `vparty_antipluralism_v2`

| Field | Value |
|---|---|
| Prompt name | `vparty_antipluralism_v2` |
| Prompt version | `v2` |
| Rubric version | `vparty@2020 + vdem-attacks-on@v13` (V-Party 2020 release; V-Dem v13 attacks-on indicators) — unchanged from v1 |
| Schema reference | `discourse-analysis-schema.md` Q3 (illiberal axis), Q4, Q5 |
| Output schema | `prompts/vparty_antipluralism_v2.schema.json` (`$id` bumped; field shape identical to v1) |
...
| Changes vs v1 | `judiciary_attack` scope tightened with explicit institutional allowlist + administrative-agency denylist (closes the v1 ANRP-mockery false positive surfaced on the 30-speech Flash-Lite smoke). No marker added or removed; no scoring rule changed. Schema field shape identical; only `$id` bumped. |
...
The judicial branch, prosecutors, the Constitutional Court, or specific named judicial bodies are delegitimised as a class — portrayed as politicised, corrupt, controlled by external forces, illegitimate, or a "parallel state." Criticism of a specific decision or a specific judge is NOT this marker — the marker requires the institutional category to be delegitimised.

**Scope (allowlist — judicial bodies in V-Party / V-Dem terms).** This marker is reserved for delegitimisation of the following institutions and their members:

- `judecători` (judges) as a class; specific named judges only when the framing extends to "judges generally"
- `procurori` (prosecutors) / `parchet` / `Parchetul` / `Parchetul General` (the prosecution service) as a class
- `Direcția Națională Anticorupție` / **DNA** / `Procurorul Șef DNA`
- `DIICOT` (organised-crime/terrorism prosecution)
- `Curtea Constituțională` / **CCR** (Constitutional Court)
- `Înalta Curte de Casație și Justiție` / **ICCJ** (High Court)
- `Consiliul Superior al Magistraturii` / **CSM** (Superior Council of the Magistracy)
- `Inspecția Judiciară`
- `justiția` (justice / judiciary) when used as a systemic-institution noun, NOT as "the legal system in general" or "fairness"

**Out of scope (denylist — administrative agencies, NOT judicial).** The following are **administrative agencies, regulatory bodies, or executive authorities** — NOT part of the judiciary. Mocking, renaming, or attacking them does NOT fire `judiciary_attack`:

- **ANRP** (Agenția Națională pentru Restituirea Proprietăților) — property restitution
- **ANAF** (Agenția Națională de Administrare Fiscală) — tax administration
- **ANI** (Agenția Națională de Integritate) — integrity agency
- **ANPC** (Autoritatea Națională pentru Protecția Consumatorilor) — consumer protection
- **ASF** (Autoritatea de Supraveghere Financiară) — financial supervision
- **ANRE** (Autoritatea Națională de Reglementare în Energie) — energy regulation
- **ANCOM** (Autoritatea Națională pentru Comunicații) — telecoms regulation
- **STS** (Serviciul de Telecomunicații Speciale) — special telecoms
- **AEP** (Autoritatea Electorală Permanentă) — electoral authority (partial overlap with `democratic_norms_rejection` if the speech rejects elections wholesale)
- **SRI / SIE / DGIA / SPP** — intelligence services (not part of the judiciary; these may surface elsewhere as conspiracy/security framings)
- Ministries in general (Ministerul Justiției, Ministerul Finanțelor, etc.) — even Ministerul Justiției is the executive, not the judiciary

**Why the denylist matters.** A speech that mocks ANRP (e.g., the v1-smoke false positive: "Agenția Națională pentru Hoție") is engaging in creative-insult rhetoric against an administrative agency, NOT delegitimising the judiciary. The journalistic signal V-Party tries to capture is "the speaker rejects the constitutional independence of the judicial branch" — fundamentally a separation-of-powers concern. Renaming ANRP to mock its perceived mismanagement is partisan-attack rhetoric, not anti-pluralism. The future `insults` and `rhetorical_moves` buckets (per `discourse-analysis-schema.md`) will catch such moves; for V-Party v2, they are explicitly out of scope.

**Romanian patterns (positive).**
- `procurori politici / cumpărați / abuzivi` (as a class — judicial / prosecutorial)
- `judecători politici / la comandă`
- `DNA dictatura / dictatura procurorilor`
- `statul paralel` (when explicitly targeting judiciary — DNA, parchet, CCR; not when targeting other institutions)
- `CCR a fost capturată / aservită / politizată`
- `justiția selectivă` (as systemic claim about the judiciary)
- `desființați DNA / desființați Parchetul`
- `binomul SRI-DNA` (a specific anti-pluralist trope from 2017–2019; fires because DNA is named)
- `kovesismul`
- calls to disband / restructure / radically reform DNA / Parchetul / CCR / ICCJ / CSM
- attacks on judicial independence as a constitutional principle
...
- "Această Curte Constituțională ne-a fost cumpărată de Bruxelles, ignoră voința poporului, nu mai are nicio legitimitate." → judiciary_attack (CCR delegitimised as a class).
- "Procurorii de la noi sunt slugi ale serviciilor secrete, nu mai facem nimic în această țară până nu desființăm parchetele." → judiciary_attack.

**Not this marker (negative — administrative agencies / ministries / regulators).**
...
- **"Agenția Națională pentru Hoție" (mocking ANRP)** → ANRP is an administrative agency for property restitution, NOT a judicial body; do not emit. (Surfaced as v1 false positive on 2026-05-08 smoke; the rhetorical move is a creative insult and may surface in the future `insults` / `rhetorical_moves` buckets, but not in V-Party.)
- "ANAF este o gașcă de incompetenți care strică economia" → ANAF is the tax administration (executive); creative insult against an administrative agency, not anti-pluralism. Do not emit.
- "Acest minister al justiției a politizat numirile" → critique of an executive ministry's decisions; do not emit (the Ministry of Justice is an executive body, not the judiciary itself; `judiciary_attack` requires the *courts*/*prosecutors* to be delegitimised, not the ministry above them).
- "ASF a aprobat o listă de companii toxice" → critique of a regulator's decisions; do not emit.
- "STS controlează totul, urmărește comunicațiile politicienilor" → conspiracy framing about a non-judicial intelligence service; out of scope for this marker (may overlap with future `conspiracy_framing` bucket).

**Cross-bucket disambiguation.** If a speech mocks an administrative agency AND attacks the judiciary, emit `judiciary_attack` for the judicial-attack passages only and let the administrative-agency mockery be picked up by future markers (`insults` / `rhetorical_moves`). Do not stretch `judiciary_attack` to cover the administrative-agency rhetoric — the strict scope is what makes the marker journalistically meaningful and cross-comparable with V-Dem's `attacks_on_judiciary` country-year indicator.
...
9. **(v2) Stretching `judiciary_attack` to cover non-judicial state institutions.** Administrative agencies (ANRP, ANAF, ASF, ANRE, ANI, ANPC, ANCOM, STS, AEP, etc.), regulators, ministries, and intelligence services are NOT judicial bodies. Mockery, renaming, or critique of these institutions does NOT fire `judiciary_attack` — see the marker's allowlist / denylist sections above. The strict scope is what keeps the marker cross-comparable with V-Dem's `attacks_on_judiciary` indicator.
...
Any change to this prompt — new marker kind, refined scoring boundary, modified output schema — is a `v3` bump in a new file. `v1` and `v2` stay here permanently so codings emitted under them remain reproducible.

The output stamps `prompt_version: "vparty_antipluralism_v2"` and `rubric_version: "vparty@2020 + vdem-attacks-on@v13"` into every analysis sidecar's `extractor_versions.frameworks.vparty_antipluralism` block.

### v1 → v2 changelog

- **`judiciary_attack` scope tightening.** Surfaced on the 2026-05-08 30-speech Flash-Lite smoke: 1 of 5 V≥1 cases (Simion 2024) fired `judiciary_attack` on a mocking rename of ANRP — an administrative property-restitution agency, not a judicial body. v2 adds an explicit allowlist (judges, prosecutors, parchet, DNA, DIICOT, CCR, ICCJ, CSM, Inspecția Judiciară) and an explicit denylist (ANRP, ANAF, ANI, ANPC, ASF, ANRE, ANCOM, STS, AEP, intelligence services, ministries) with a Romanian-language disambiguation block + 6 new "Not this marker" examples. Failure-mode #9 added to the failure-modes list.
- **No other changes.** Schema field shape is byte-identical to v1 (`$id` is the only change, bumped from `vparty_antipluralism_v1` to `vparty_antipluralism_v2`). Marker enum unchanged. Score scale unchanged. Confidence calibration unchanged. Voice integration rules unchanged. Few-shot examples 1–3 unchanged.

The 8 sub-categories of v1 (12 sub-bullets in failure modes, 5 score-band confidence cells, voice integration rules, etc.) are all preserved bit-for-bit; v2 is a precision-targeted scope fix, not a rewrite.
