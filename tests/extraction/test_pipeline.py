...
def test_dispatch_skips_unimplemented_types(isolated_md, monkeypatch) -> None:
    """A doc that classifies as `plenary_stenogram` (or any non-question
    type) must skip with reason — not produce a stub `body=other` sidecar.
    """
    md_path = isolated_md("qr_2026-03-25_29.md")
    # Force the classifier off-track via override_type
    result = extract(md_path, override_type="plenary_stenogram", write=False)
