...
Step 2 of the extraction pipeline (see [`docs/extraction-schema.md`](docs/extraction-schema.md), v1.7.0). Reads converted MDs, dispatches to a per-document-type extractor, and writes a `<basename>.extraction.json` sidecar next to each MD. Document type comes from the `classify` rules. v0.1 ships extractors for **`question_register`**, **`plenary_stenogram`**, **`plenary_joint_session`**, and **`committee_synthesis`** (~98% of the corpus by document count); only `report_facsimile` is still skipped with a `not-yet-implemented` reason — it doesn't get a stub `body=other` sidecar (so re-runs after the extractor lands are clean).
